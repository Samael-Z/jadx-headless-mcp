...
`jadx-headless-mcp` is a headless MCP server fronting the JADX Android decompiler. It is a single Rust binary that embeds and spawns a Java sidecar — there is no separate plugin to install. End users do `claude mcp add jadx -- jadx-mcp --apk app.apk` and that's it.
...
**Why `JadxBasePluginLoader` and not `JadxExternalPluginsLoader`** (`bridge/src/main/java/com/zin/jadxheadless/JadxBridge.java`). The latter scans `~/.jadx/plugins/` for user-installed JARs. If the user has the original `jadx-ai-mcp` (or any other GUI plugin) installed, ServiceLoader tries to resolve a class that imports `jadx.gui.ui.MainWindow` — which is not on the classpath — and `JadxDecompiler.load()` blows up with `NoClassDefFoundError`. `JadxBasePluginLoader` reads ONLY from the bundled classpath, where we control what's there. This is the kind of bug that's "fixed locally on the dev machine" forever; don't switch back without rethinking it.
...
1. `bridge/src/main/java/com/zin/jadxheadless/server/routes/<Category>Routes.java` — handler method
2. `bridge/src/main/java/com/zin/jadxheadless/server/BridgeServer.java` — register the route
...
- **The bundled jar invalidates by size**, not by hash. `materialize_bridge_jar()` writes the bundled bytes to `<data-local>/jadx-headless-mcp/jadx-bridge-<size>.jar`. If you somehow ship a different jar with the exact same byte count, the old one won't get overwritten. Use a different version suffix or wipe the cache dir.
- **Renames persist across restarts.** They are applied via jadx's headless **code-data API** (`JadxArgs.getCodeData()` + `JadxCodeRename(JadxNodeRef, newName)` + `JadxDecompiler.reloadCodeData()`) — NOT the GUI-only `NodeRenamedByUser` event, which has no listener in a headless decompiler and was a silent no-op (renames appeared to succeed but changed nothing). Each rename is journaled to `<apk-dir>/.jadx-mcp-cache/<apk>.<size>.renames.json` and replayed on the next load. See `BridgeContext.applyAndRecordRename` / `loadAndReplayRenames`. Variable renames are still unsupported.
